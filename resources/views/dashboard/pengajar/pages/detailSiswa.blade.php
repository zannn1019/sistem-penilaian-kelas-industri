        ...
        <div class="w-full h-full px-7 max-sm:px-0 flex flex-col">
            <div
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <span class="border-l px-2 text-xs capitalize">{{ $info_kelas->tahun_ajar }}</span>
                </div>
                <div class="flex gap-2 items-center">
                    <i class="fa-solid fa-book text-xl"></i>
                    <span class="font-semibold">Semester</span>
                    <span class="border-l px-2 text-xs capitalize">{{ $info_kelas->semester }}</span>
                </div>
                <div class="flex gap-2 items-center">
                            ...
                            <div class="bg-darkblue-100 border rounded-3xl border-black overflow-hidden">
                                ...
                                @foreach ($daftar_mapel as $mapel)
                                    @php
                                        ...
                                        foreach (
                                            $mapel->tugas
                                                ->where('id_kelas', $info_kelas->id)
                                                ->where('tahun_ajar', $info_kelas->tahun_ajar)
                                                ->where('semester', $info_kelas->semester)
                                            as $tugas
                                        ) {
                                            $nilai = $tugas->nilai
                                                ->where('id_siswa', $info_siswa->id)
                                    ...
                                    @if (request('view') == 'full')
                                        <div tabindex="0"
                                            class="collapse rounded-none h-max border-t border-black  overflow-x-auto {{ $avgNilai == '-' ? 'bg-red-500' : '' }}">
                                            <div class="collapse-title p-0 flex h-max text-center text-sm w-full min-h-0">
                                                ...
                                                <h1 class="flex justify-center items-center flex-grow">
                                                    @if ($avgNilai != '-')
                                                        {{ $mapel->tugas->where('tahun_ajar', $info_kelas->tahun_ajar)->where('semester', $info_kelas->semester)->avg(function ($tugas) use ($info_siswa) {
                                            ...
                                            <div
                                                ...
                                                @if ($mapel->tugas->where('id_kelas', $info_kelas->id)->where('tahun_ajar', $info_kelas->tahun_ajar)->where('semester', $info_kelas->semester)->count())
                                                    @foreach ($mapel->tugas->where('id_kelas', $info_kelas->id)->where('tahun_ajar', $info_kelas->tahun_ajar)->where('semester', $info_kelas->semester) as $tugas)
                                                        <div
                                                            class="p-0 flex h-max text-center text-sm w-full min-h-0 border-b border-black">
                                                            <h1
                                                            ...
                                                            <div
                                                                ...
                                                                <div class="w-full flex">
                                                                    <h1 class="w-20 border-r border-black py-2">
                                                                        {{ $tugas->nilai->where('id_siswa', $info_siswa->id)->where('id_siswa', $info_siswa->id)->where('tahun_ajar', $info_siswa->kelas->tahun_ajar)->where('semester', $info_siswa->kelas->semester)->value('nilai') ?? '-' }}
                                                                    </h1>
                                                                    <h1
                                                                        class="flex-grow w-full border-black py-2 capitalize">
                                                                        {{ $tugas->nilai->where('id_siswa', $info_siswa->id)->where('id_siswa', $info_siswa->id)->where('tahun_ajar', $info_siswa->kelas->tahun_ajar)->where('semester', $info_siswa->kelas->semester)->value('nilai') != null
                                                                            ? NumberToWords::transformNumber('id', $tugas->nilai->where('id_siswa', $info_siswa->id)->value('nilai'))
                                                                            : '-' }}
                                                                    </h1>
                                                                </div>
                                                            </div>
                                                            <h1
                                                                ...
                                                                @if ($tugas->nilai->where('id_siswa', $info_siswa->id)->where('id_siswa', $info_siswa->id)->where('tahun_ajar', $info_siswa->kelas->tahun_ajar)->where('semester', $info_siswa->kelas->semester)->value('nilai') != null)
                                                                    {{ $tugas->nilai->where('id_siswa', $info_siswa->id)->where('id_siswa', $info_siswa->id)->where('tahun_ajar', $info_siswa->kelas->tahun_ajar)->where('semester', $info_siswa->kelas->semester)->value('nilai') <= 75
                                                                        ? 'Nilai kurang'
                                                                        : 'Terlampaui' }}
                                                                @else
                                                                    -
                                                    ...
                                                    @foreach ($mapel->tugas->where('id_kelas', $info_kelas->id)->where('tahun_ajar', $info_kelas->tahun_ajar)->where('semester', $info_kelas->semester) as $tugas)
                                                        <div class="grid grid-cols-3 text-center text-sm w-full ">
                                                            ...
                                                            <h1
                                                                ...
                                                                {{ $tugas->nilai->where('id_siswa', $info_siswa->id)->where('id_siswa', $info_siswa->id)->where('tahun_ajar', $info_siswa->kelas->tahun_ajar)->where('semester', $info_siswa->kelas->semester)->value('nilai') ?? 'Belum Dinilai' }}
                                                            </h1>
                                                        </div>
                                                    @endforeach
                                                @else
